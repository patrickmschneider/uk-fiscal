...
Deficit charts: bold net-total overlays, components/net-only selector, history since 2000 (1990 option), and labelled forecast background shading. Primary and net-interest history now extends to 1990–91, calculated from ONS using OBR Working Paper 17 table A.1 (JW2P−JW2L+JW2M). Both deficit decompositions now use matched historical OBR databank aggregates from 1975–76, with selected forecast-vintage rows taking precedence from their first year. The user-supplied August 2026 workbook is tracked for reproducible refreshes; its SHA-256 and source sheets are recorded in the output. OBR databank downloads returned HTTP 403 during this update.
